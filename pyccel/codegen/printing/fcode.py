...
class FCodePrinter(CodePrinter):
        ...
        while container:
            if cls_name in container.classes:
                ...
                if method_name in methods:
                    ...
                    interface_funcs = {
                        f.name: f for i in cls.interfaces for f in i.functions
                    }
                    if method_name in interface_funcs:
                        return interface_funcs[method_name]
                    errors.report(
                        UNDEFINED_METHOD, symbol=method_name, severity="fatal"
                    )
            container = container.parent_scope
        if isinstance(method_name, DottedName):
            return self.get_function(DottedName(method_name.name[1:]))
        errors.report(UNDEFINED_FUNCTION, symbol=method_name, severity="fatal")
    ...
    def _define_gFTL_element(self, element_type, imports_and_macros, element_name):
        ...
        if isinstance(element_type, FixedSizeNumericType):
            ...
            if isinstance(element_type.primitive_type, PrimitiveComplexType):
                complex_tool_import = Import(
                    "pyc_tools_f90", Module("pyc_tools_f90", (), ())
                )
                self.add_import(complex_tool_import)
                imports_and_macros.append(complex_tool_import)
                compare_func = FunctionDef(
                    "complex_comparison",
            ...
            defs = [
                MacroDefinition(element_name, element_type.primitive_type),
                MacroDefinition(
                    f"{element_name}_KINDLEN(context)", KindSpecification(element_type)
                ),
                MacroDefinition(f"{element_name}_LT(x,y)", lt_def),
                MacroDefinition(
                    f"{element_name}_EQ(x,y)",
                    PyccelAssociativeParenthesis(PyccelEq(tmpVar_x, tmpVar_y)),
                ),
            ]
            undefs = [
                MacroUndef(element_name),
        ...
        if isinstance(element_type, (NumpyNDArrayType, HomogeneousTupleType)):
            ...
            raise NotImplementedError(
                "Support for containers of types defined in other modules is not yet implemented"
            )
        return defs, undefs
    ...
    def _build_gFTL_module(self, expr_type):
        ...
        matching_expr_type = next(
            (t for t in self._generated_gFTL_types if expr_type == t), None
        )
            ...
            if isinstance(expr_type, (HomogeneousListType, HomogeneousSetType)):
                ...
                if isinstance(expr_type, HomogeneousSetType):
                    ...
                    if not isinstance(element_type, FixedSizeNumericType):
                        raise NotImplementedError(
                            "Support for sets of types which define their own < operator is not yet implemented"
                        )
                else:
                    type_name = "Vector"
                imports_and_macros = []
                defs, undefs = self._define_gFTL_element(
                    element_type, imports_and_macros, "T"
                )
                imports_and_macros.extend(
                    [
                        *defs,
                        MacroDefinition(type_name, expr_type),
                        MacroDefinition(
                            f"{type_name}Iterator", IteratorType.get_new(expr_type)
                        ),
                        Import(
                            LiteralString(f"{type_name.lower()}/template.inc"),
                            Module("_", (), ()),
                        ),
                ...
                if not isinstance(key_type, FixedSizeNumericType):
                    raise NotImplementedError(
                        "Support for dicts whose keys define their own < operator is not yet implemented"
                    )
                key_defs, key_undefs = self._define_gFTL_element(
                    key_type, imports_and_macros, "Key"
                )
                val_defs, val_undefs = self._define_gFTL_element(
                    value_type, imports_and_macros, "T"
                )
                imports_and_macros.extend(
                    [
                        *key_defs,
    ...
    def _build_gFTL_extension_module(self, expr_type):
        ...
        matching_expr_type = next(
            (m for t, m in self._generated_gFTL_types.items() if expr_type == t), None
        )
        # Get the module describing the extension
        matching_expr_extensions = next(
            (m for t, m in self._generated_gFTL_extensions.items() if expr_type == t),
            None,
        )
        typename = self._print(expr_type)
        ...
        if matching_expr_extensions:
            ...
            if matching_expr_type is None:
                ...
                imports_and_macros.append(
                    Import(f"gFTL_extensions/{mod_name}", type_module)
                )
            ...
            if isinstance(expr_type, HomogeneousSetType):
                ...
                imports_and_macros += [
                    (
                        Import(LiteralString("Set_extensions.inc"), Module("_", (), ()))
                        if getattr(i, "source", None) == set_filename
                        else i
                    )
                ...
                imports_and_macros += [
                    (
                        Import(
                            LiteralString("Vector_extensions.inc"), Module("_", (), ())
                        )
                        if getattr(i, "source", None) == vector_filename
                        else i
                    )
                    for i in type_module.imports
                ]
                self.add_import(
                    Import(
                        "gFTL_functions/Vector_extensions",
                        Module("_", (), ()),
                        ignore_at_print=True,
                    )
                )
                ...
                imports_and_macros += [
                    (
                        Import(LiteralString("Map_extensions.inc"), Module("_", (), ()))
                        if getattr(i, "source", None) == map_filename
                        else i
                    )
                    for i in type_module.imports
                ]
    ...
    def _print_Import(self, expr):
        ...
        for i in targets:
            ...
            if old_name != new_name:
                ...
                raise TypeError(
                    "Expecting str, PyccelSymbol, DottedName or AsName, "
                    "given {}".format(type(i))
                )
    ...
    def _print_PythonPrint(self, expr):
        ...
        for f in expr.expr:
            if f.has_keyword:
                if f.keyword == "sep":
                    ...
                    errors.report(
                        "{} not implemented as a keyworded argument".format(f.keyword),
                        severity="fatal",
                    )
        args_format = []
        args = []
        ...
        tuple_tags = (
            FunctionCallArgument(LiteralString("(")),
            FunctionCallArgument(LiteralString(")")),
        )
        list_tags = (
            FunctionCallArgument(LiteralString("[")),
        ...
        for i, f in enumerate(orig_args):
            ...
            if isinstance(f, (PythonTuple, PythonList, PythonSet, str)):
                start_tag, end_tag = (
                    set_tags
                    if isinstance(f, PythonSet)
                    else list_tags if isinstance(f, PythonList) else tuple_tags
                )
                if args_format:
                    code += self._formatted_args_to_print(
                        args_format, args, sep, separator, expr
                    )
                    args_format = []
                    args = []
                ...
                args = [
                    FunctionCallArgument(print_arg)
                    for tuple_elem in f
                    for print_arg in (tuple_elem, container_sep)
                ][:-1]
                if len(f) == 1:
                    args.append(FunctionCallArgument(LiteralString(",")))
                code += self._print(
                    PythonPrint(
                        [start_tag, *args, end_tag, empty_sep, end_of_tuple],
                        file=expr.file,
                    )
                )
                args = []
                ...
                isinstance(f.class_type, (TupleType, HomogeneousContainerType))
                and not isinstance(f.class_type, StringType)
                ...
                for_index = self.scope.get_temporary_variable(
                    PythonNativeInt(), name="i"
                )
                ...
                for_end = FunctionCallArgument(for_end_char, keyword="end")

                if_body = [
                    PythonPrint(
                        [FunctionCallArgument(f[max_index]), empty_end], file=expr.file
                    )
                ]
                if_block = If(
                    IfSection(
                        PyccelGt.make_simplified(f.shape[0], LiteralInteger(0)), if_body
                    )
                )
                ...
                body = CodeBlock(
                    [
                        PythonPrint(
                            [FunctionCallArgument(LiteralString("[")), empty_end],
                            file=expr.file,
                        ),
                        for_loop,
                        if_block,
    ...
    def _formatted_args_to_print(self, fargs_format, fargs, fend, fsep, expr):
        ...
        if expr.file == "stderr":
            self._constantImports[-1].setdefault("ISO_FORTRAN_ENV", set()).add(
                ("stderr", "error_unit")
            )
        ...
        return (
            f"write(stdout, '({args_formatting})', advance=\"{advance}\") {args_code}\n"
        )
    ...
    def _get_print_format_and_arg(self, var, var_code=None):
        ...
        if isinstance(var.class_type, StringType):
            ...
            isinstance(var, FunctionCall) and len(var.funcdef.results) > 1
        ) or isinstance(var.class_type, InhomogeneousTupleType):
            var_elem_code = var_code[1:-1].split(", ")
            args_and_formats = [
                self._get_print_format_and_arg(v.var, c)
                for v, c in zip(var.funcdef.results, var_elem_code)
            ]
            ...
            args_and_formats = [
                self._get_print_format_and_arg(var.funcdef.results.var, var_code)
            ]
            formats = ',", ",'.join(af[0] for af in args_and_formats)
            arg_format = f'"(",{formats},")"'
            arg = ", ".join(af[1] for af in args_and_formats)
            ...
            if isinstance(var_type.primitive_type, PrimitiveComplexType):
                ...
                errors.report(
                    f"Printing {var_type} type is not supported currently",
                    severity="fatal",
                )
        else:
            errors.report(
                f"Printing {var_type} type is not supported currently", severity="fatal"
            )
    ...
    def _print_PythonRound(self, expr):
        ...
        ndigits = (
            self._apply_cast(NumpyInt64Type(), expr.ndigits)
            if expr.ndigits
    ...
    def _print_PythonTuple(self, expr):
        ...
        if elements:
            ...
            try:
                default = self._print(
                    convert_to_literal(0, expr.class_type.element_type)
                )
            except TypeError:
                errors.report(
                    f"Can't create an empty tuple with elements of type {expr.class_type.element_type}",
                    severity="fatal",
                    symbol=expr,
                )
    ...
    def _print_PythonList(self, expr):
        if len(expr.args) == 0:
            ...
            if assign:
                ...
                raise errors.report(
                    "Can't use an empty list without assigning it to a variable as the type cannot be deduced",
                    severity="fatal",
                    symbol=expr,
                )
    ...
    def _print_PythonSet(self, expr):
        if len(expr.args) == 0:
            ...
            if assign:
                ...
                raise errors.report(
                    "Can't use an empty set without assigning it to a variable as the type cannot be deduced",
                    severity="fatal",
                    symbol=expr,
                )
    ...
    def _print_PythonDict(self, expr):
        if len(expr) == 0:
            ...
            if assign:
                ...
                raise errors.report(
                    "Can't use an empty dict without assigning it to a variable as the type cannot be deduced",
                    severity="fatal",
                    symbol=expr,
                )
            ...
            pair_type = self._print(
                PairType.get_new(class_type.key_type, class_type.value_type)
            )
            args = ", ".join(
                f"{pair_type}({self._print(k)}, {self._print(v)})" for k, v in expr
            )
            list_arg = f"[{args}]"
    ...
    def _print_Constant(self, expr):
        if expr == math_constants["nan"]:
            errors.report("Can't print nan in Fortran", severity="error", symbol=expr)
    ...
    def _print_ListPop(self, expr):
        ...
        parent_assign_nodes = expr.get_direct_user_nodes(
            lambda u: isinstance(u, Assign)
        )
        ...
        if index_element:
            ...
            if isinstance(index_element, PyccelUnarySub) and isinstance(
                index_element.args[0], LiteralInteger
            ):
                index_element = PyccelMinus.make_simplified(
                    _shape, index_element.args[0]
                )
            tmp_iter = self.scope.get_temporary_variable(
                IteratorType.get_new(list_obj.class_type), name=f"{list_obj}_iter"
            )
            code = (
                f"{tmp_iter} = {target} % begin() + {self._print(index_element)}\n"
                f"{lhs} = {tmp_iter} % of()\n"
    ...
    def _print_SetUnion(self, expr):
        ...
        for arg in expr.args:
            if arg.class_type == expr_type:
                ...
                args_insert.append(
                    f"call {result} % insert({a} % begin(), {a} % end())\n"
                )
            else:
    ...
    def _print_SetIntersectionUpdate(self, expr):
        ...
        # See pyccel/stdlib/gFTL_functions/Set_extensions.inc for the definition
        return "".join(
            f"call {type_name}_intersection_update({var_code}, {self._print(arg)})\n"
            for arg in expr.args
        )
    ...
    def _print_SetDifferenceUpdate(self, expr):
        ...
        # See pyccel/stdlib/gFTL_functions/Set_extensions.inc for the definition
        return "".join(
            f"call {type_name}_difference_update({var_code}, {self._print(arg)})\n"
            for arg in expr.args
        )
    ...
    def _print_NumpyNorm(self, expr):
        ...
        if order == 2:
            ...
            order.dtype.primitive_type,
            (PrimitiveIntegerType, PrimitiveFloatingPointType),
        ):
            pow_factor = self._apply_cast(
                expr.dtype, PyccelDiv.make_simplified(LiteralInteger(1), order)
            )
    ...
    def _print_NumpyLinspace(self, expr):
        ...
        end = self._print(PyccelMinus.make_simplified(expr.num, LiteralInteger(1)))
        endpoint_code = ""
        ...
        if not isinstance(expr.endpoint, LiteralFalse):
            ...
            if expr.rank > 1:
                ...
                lhs_source.substitute(
                    expr.ind, PyccelMinus.make_simplified(expr.num, LiteralInteger(1))
                )
                ...
                lhs = self._print(
                    IndexedElement(
                        lhs, PyccelMinus.make_simplified(expr.num, LiteralInteger(1))
                    )
                )
    ...
    def _print_NumpyCountNonZero(self, expr):
        ...
        if axis is None:
            ...
            if array.order == "C":
                f_dim = PyccelMinus.make_simplified(
                    LiteralInteger(array.rank), expr.axis
                )
            else:
                f_dim = PyccelAdd.make_simplified(expr.axis, LiteralInteger(1))
    ...
    def _print_NumpyWhere(self, expr):
        ...
        stmt = "merge({true}, {false}, {cond})".format(
            true=value_true, false=value_false, cond=condition
        )
        ...
        if expr.rank <= 2:
            ...
            for a in expr_args:
                ...
                if a.order == inv_order:
                    ...
                    order_code = ", ".join(
                        self._print(LiteralInteger(i)) for i in range(a.rank, 0, -1)
                    )
            ...
            if len(new_args) != 1 or expr.arg.rank < expr.rank:
                if order == "C":
                    ...
                    rhs_code = (
                        f"reshape({rhs_code}, [{shape_code}], order=[{order_code}])"
                    )
    ...
    def _print_PyccelArrayShapeElement(self, expr):
        ...
        if isinstance(arg.class_type, (NumpyNDArrayType, HomogeneousTupleType)):
            ...
            if arg.order == "C":
                index = PyccelMinus.make_simplified(
                    LiteralInteger(arg.rank), expr.index
                )
                index = self._print(index)
            else:
                index = PyccelAdd.make_simplified(expr.index, LiteralInteger(1))
                index = self._print(index)

            return f"size({arg_code}, {index}, {prec})"
        elif isinstance(
            arg.class_type, (HomogeneousListType, HomogeneousSetType, DictType)
        ):
    ...
    def _print_PythonMinMax(self, expr):
        ...
        if isinstance(arg, Variable):
            if isinstance(arg.class_type, (HomogeneousListType, HomogeneousSetType)):
                ...
                raise TypeError(
                    f"Expecting a variable of type list or set, given {arg.class_type}"
                )
        else:
            arg_code = self._get_node_without_gFTL(arg)
    ...
    def _print_Declare(self, expr):
        ...
        if isinstance(
            ...
            if var.is_argument:
                ...
                arg = var.get_direct_user_nodes(
                    lambda u: isinstance(u, FunctionDefArgument)
                )[0]
            ...
            expr_type, (NumpyNDArrayType, HomogeneousTupleType, FixedSizeType)
        ):
            ...
            if rank > 0:
                ...
                if intent_in:
                    ...
                    ubounds = [
                        PyccelMinus.make_simplified(s, LiteralInteger(1))
        ...
        if not is_static:
            if is_alias:
                ...
                and not intent_in
                and isinstance(
                    var.class_type, (NumpyNDArrayType, HomogeneousTupleType, StringType)
                )
            ):
    ...
    def _print_CodeBlock(self, expr):
        if not expr.unravelled:
            body_exprs = expand_to_loops(
                expr,
                self.scope.get_temporary_variable,
                self.scope,
    ...
    def _print_Assign(self, expr):
        ...
        if isinstance(lhs, Variable) and isinstance(lhs.dtype, SymbolicType):
            return ""
    ...
    def _print_Allocate(self, expr):
        ...
        if expr.alloc_type == "function":
            if isinstance(
                class_type, (NumpyNDArrayType, HomogeneousTupleType, CustomDataType)
            ):
                ...
                isinstance(class_type, HomogeneousListType)
                and expr.status != "unallocated"
            ):
        ...
        if isinstance(
            ...
            shape_code = ", ".join(
                "0:" + self._print(PyccelMinus.make_simplified(i, LiteralInteger(1)))
                for i in shape
            )
            if shape:
            ...
            if expr.alloc_type == "resize":
                ...
                return (
                    self._print(Deallocate(expr.variable))
    ...
    def _print_Deallocate(self, expr):
        ...
        if var.is_alias or isinstance(class_type, DictType):
            ...
            class_type, (NumpyNDArrayType, HomogeneousTupleType, StringType)
        ):
            var_code = self._print(var)
    ...
    def _print_PairType(self, expr):
        return (
            "Pair_" + self._print(expr.key_type) + "__" + self._print(expr.value_type)
        )
    ...
    def _print_PythonRange(self, expr):
        ...
        if isinstance(test_step, Literal):
            if isinstance(expr.step, PyccelUnarySub):
                stop = PyccelAdd.make_simplified(expr.stop, LiteralInteger(1))
            else:
                stop = PyccelMinus.make_simplified(expr.stop, LiteralInteger(1))
        else:
            stop = IfTernaryOperator(
                PyccelGt(expr.step, LiteralInteger(0)),
                PyccelMinus.make_simplified(expr.stop, LiteralInteger(1)),
                PyccelAdd.make_simplified(expr.stop, LiteralInteger(1)),
            )
    ...
    def _print_For(self, expr):
        ...
        if isinstance(
            iterable, (VariableIterator, DictItems, DictKeys, DictValues)
        ) and isinstance(iterable.variable.class_type, (DictType, HomogeneousSetType)):
            ...
            if isinstance(var, Variable):
                ...
                suggested_name = ""
                errors.report(
                    "Iterating over a temporary object. This may cause compilation issues or cause calculations to be carried out twice",
                    severity="warning",
                    symbol=expr,
                )
            var_code = self._print(var)
            iterator = self.scope.get_temporary_variable(
                IteratorType.get_new(iterable_type), name=suggested_name + "iter"
            )
            last = self.scope.get_temporary_variable(
                IteratorType.get_new(iterable_type), name=suggested_name + "last"
            )
            if isinstance(iterable, DictItems):
                key = self._print(expr.target[0])
                val = self._print(expr.target[1])
                target_assign = (
                    f"{key} = {iterator} % first()\n" f"{val} = {iterator} % second()\n"
                )
    ...
    def _print_While(self, expr):
        ...
        return ("do while ({test})\n" "{body}" "end do\n").format(
            test=self._print(expr.test), body=body
        )
    ...
    def _print_PyccelIs(self, expr):
        ...
        if all(
            isinstance(var.dtype.primitive_type, PrimitiveBooleanType) for var in (a, b)
        ):
    ...
    def _print_PyccelEq(self, expr):
        ...
        if all(isinstance(var, PrimitiveBooleanType) for var in (a, b)):
            ...
            errors.report(PYCCEL_RESTRICTION_TODO, symbol=expr, severity="error")
            return ""
    ...
    def _print_PyccelIn(self, expr):
        ...
        if isinstance(container_type, (HomogeneousSetType, DictType)):
            ...
            raise errors.report(PYCCEL_RESTRICTION_TODO, symbol=expr, severity="fatal")
    ...
    def _print_SysExit(self, expr):
        ...
        if isinstance(exit_code, LiteralInteger):
            ...
            not isinstance(
                getattr(exit_code.dtype, "primitive_type"), PrimitiveIntegerType
            )
            or exit_code.rank > 0
        ):
    ...
    def _print_NumpyUfuncBase(self, expr):
        ...
        if func_name.startswith("ieee_"):
            self._constantImports[-1].setdefault("ieee_arithmetic", set()).add(
                func_name
            )
        args = [
            self._get_node_without_gFTL(
                NumpyFloat(a)
                if isinstance(a.dtype.primitive_type, PrimitiveIntegerType)
                else a
            )
            for a in expr.args
        ]
    ...
    def _print_NumpyIsInf(self, expr):
        code = PyccelAssociativeParenthesis(
            PyccelAnd(
                PyccelNot(NumpyIsFinite(expr.arg)), PyccelNot(NumpyIsNan(expr.arg))
            )
        )
    ...
    def _print_NumpyTranspose(self, expr):
        ...
        if assigns and assigns[0].lhs.order != var.order:
            ...
            order = ", ".join(
                self._print(LiteralInteger(i)) for i in range(var.rank, 0, -1)
            )
    ...
    def _print_IndexedElement(self, expr):
        ...
        while isinstance(base, IndexedElement) and isinstance(
            base.class_type, (HomogeneousTupleType, NumpyNDArrayType)
        ):
        ...
        for i, ind in enumerate(inds):
            _shape = PyccelArrayShapeElement(
                base, i if expr.base.order != "C" else len(inds) - i - 1
            )
            if isinstance(ind, Slice):
                inds[i] = self._new_slice_with_processed_arguments(
                    ind, _shape, allow_negative_indexes
                )
            elif isinstance(ind, PyccelUnarySub) and isinstance(
                ind.args[0], LiteralInteger
            ):
                inds[i] = PyccelMinus.make_simplified(_shape, ind.args[0])
            else:
                # indices of indexedElement of len==1 shouldn't be a tuple
                if isinstance(ind, tuple) and len(ind) == 1:
                    inds[i] = ind[0]
                if allow_negative_indexes and not isinstance(ind, LiteralInteger):
                    inds[i] = IfTernaryOperator(
                        PyccelLt(ind, LiteralInteger(0)),
                        PyccelAdd.make_simplified(_shape, ind),
                        ind,
                    )
        ...
        if isinstance(base.class_type, HomogeneousListType):
            ...
            ind_code = self._print(
                PyccelAdd.make_simplified(inds[0], LiteralInteger(1))
            )
            ...
            errors.report(
                f"Don't know how to index type {base.class_type}",
                symbol=expr,
                severity="fatal",
            )
            return ""
    ...
    def _new_slice_with_processed_arguments(_slice, array_size, allow_negative_index):
        ...
        if isinstance(start, PyccelUnarySub) and isinstance(
            start.args[0], LiteralInteger
        ):
            start = PyccelMinus.make_simplified(array_size, start.args[0])
        elif (
            start is not None
            and allow_negative_index
            and not isinstance(start, LiteralInteger)
        ):
            start = IfTernaryOperator(
                PyccelLt(start, LiteralInteger(0)),
                PyccelAdd.make_simplified(array_size, start),
                start,
            )

        if isinstance(stop, PyccelUnarySub) and isinstance(
            stop.args[0], LiteralInteger
        ):
            stop = PyccelMinus.make_simplified(array_size, stop.args[0])
        elif (
            stop is not None
            and allow_negative_index
            and not isinstance(stop, LiteralInteger)
        ):
            stop = IfTernaryOperator(
                PyccelLt(stop, LiteralInteger(0)),
                PyccelAdd.make_simplified(array_size, stop),
                stop,
            )

        # negative step in slice
        if isinstance(step, PyccelUnarySub) and isinstance(
            step.args[0], LiteralInteger
        ):
            stop = (
                PyccelAdd.make_simplified(stop, LiteralInteger(1))
                if stop is not None
                else LiteralInteger(0)
            )
            start = (
                start
                if start is not None
                else PyccelMinus.make_simplified(array_size, LiteralInteger(1))
            )
        ...
        # variable step in slice
            ...
            if start is None:
                start = IfTernaryOperator(
                    PyccelGt(step, LiteralInteger(0)),
                    LiteralInteger(0),
                    PyccelMinus.make_simplified(array_size, LiteralInteger(1)),
                )
            ...
            stop = PyccelMinus.make_simplified(stop, LiteralInteger(1))
    ...
    def _print_FunctionCall(self, expr):
        ...
        parent_assign = expr.get_direct_user_nodes(
            lambda x: isinstance(x, (Assign, AliasAssign))
        )
    ...
    def _print_PyccelInternalFunction(self, expr):
        if isinstance(expr, NumpyNewArray):
            return errors.report(
                ALLOCATABLE_IN_EXPRESSION, symbol=expr, severity="fatal"
            )
        else:
            return self._print_not_supported(expr)
